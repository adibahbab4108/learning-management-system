...
const AddCourse = () => {
    ...
    const handleChapter = (action, chapterId = null) => {
        if (action === 'add') {
            ...
            setChapters(prev => prev.filter(chap => chap.chapterId !== chapterId));
        } else if (action === 'toggle') {
            setChapters(prev => prev.map(chap =>
                chap.chapterId === chapterId ? { ...chap, collapsed: !chap.collapsed } : chap
            ));
        }
    };
        ...
        if (action === 'add') {
            ...
            setChapters(prev => prev.map(chap =>
                chap.chapterId === chapterId ? {
                    ...chap,
                    chapterContent: chap.chapterContent.filter((_, idx) => idx !== lectureIndex)
                } : chap
            ));
        }
    };
    ...
    return (
        <div className="h-screen  overflow-scroll flex flex-col items-start justify-between p-4 md:p-8 pt-8 pb-0">
            <form onSubmit={handleSubmit} className="w-full space-y-4">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <label htmlFor="thumbnailImage" className="cursor-pointer">
                            <img src={assets.file_upload_icon} className="p-3 bg-blue-500 rounded" alt="Upload" />
                        </label>
                        <input type="file" id="thumbnailImage" name="image" onChange={handleInputChange} accept="image/*" hidden />
                        {formData.image && <img src={URL.createObjectURL(formData.image)} className="max-h-10" alt="Preview" />}
                    </div>
                </div>
                <div>
                    <p>Discount %</p>
                                    ...
                                    {chapter.chapterContent.map((lecture, lectureIndex) => (
                                        <div key={lecture.lectureId} className="flex justify-between items-center mb-2">
                                            <span>
                                                ...
                                                <a href={lecture.lectureUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 underline"> Link </a>
                                                - {lecture.isPreviewFree ? 'Free Preview' : 'Paid'}
                                            </span>
                                            <img onClick={() => handleLecture('remove', chapter.chapterId, lectureIndex)} src={assets.cross_icon} className="cursor-pointer" alt="Remove" />
                                        </div>
                                    ))}
                                    <div className="text-blue-600 cursor-pointer" onClick={() => handleLecture('add', chapter.chapterId)}>+ Add Lecture</div>
                                </div>
                            )}
                        </div>
                    ))}
                    <div onClick={() => handleChapter('add')} className="text-center bg-blue-100 p-2 rounded-lg cursor-pointer">
                        + Add Chapter
                    </div>
                </div>
                {showPopup && (
                    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50 backdrop-blur-sm">
                        <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 relative ">
                            <h2 className="text-2xl font-bold text-gray-800 mb-4 text-center">🎓 Add New Lecture</h2>
                            ...
                            <div className="space-y-4">
                                ...
                                {/* Lecture URL */}
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">Lecture Video URL</label>
                                    <input
                                        type="text"
                                        className="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                ...
                <button type="submit" className="bg-green-600 text-white py-2 px-4 rounded">Submit Course</button>
            </form>
        </div>
    );
};
